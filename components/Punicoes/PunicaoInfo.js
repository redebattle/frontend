export default function PunicaoAtiva({
  cor,
  opacity,
  motivo,
  termino,
  ...
  return (
    <>
      <div
        className={`flex lg:flex-row sm:flex-col justify-between lg:items-center sm:items-start p-4 ${cor} ${
          opacity && 'bg-opacity-25'
        } rounded-lg border-b-2 border-black sm:text-xs lg:text-sm`}
      >
        <p className="sm:mb-3">
          Motivo: <br />
          {motivo}
        ...
        <p className="sm:mb-3">
          Término: <br />
          {termino <= 0 && (
            <span className="badge badge-outline text-red-400 font-bold">
              Permanente
            </span>
          )}
        ...
        <p className="sm:mb-3">
          ...
          <div className="flex lg:justify-between sm:justify-start items-center">
            <img
              src={`https://minotar.net/avatar/${autor}/25`}
